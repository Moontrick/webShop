...
class UserDataBase:
    ...
    async def post_new_user(self, username: str, email: str, password: str):
        query = """
        ...
        """
        try:
            return await self.pool.fetchrow(query, username, email, password)
        except Exception as e:
            print(f"[DB ERROR] post_new_user: {e}")
